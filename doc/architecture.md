...
`scripts/check-production-readiness.mjs` expose deux profils fermés. Le profil `local` exécute une seule fois qualité, typecheck, build, unitaires, Foundation, Auth, Services, Galerie, Contact, SEO, accessibilité, sécurité et scan de secrets. Dans ce profil seulement, Foundation ne rappelle pas son ancien sous-gate Galerie : l'orchestrateur en conserve l'unique exécution et sa preuve propre. Sur un hôte où Firefox Playwright natif est incompatible, ce profil reproductible borne les E2E hôte à Chromium/WebKit ; la preuve Firefox automatisée est alors produite avec l'image officielle Playwright et les trois moteurs restent obligatoires dans le profil preview final. Le contrôle SEO local vérifie volontairement le contexte non indexable sur loopback ; il ne marque pas comme passée la validation Schema.org externe, qui reste `not_run` jusqu'à la preview. Le profil `preview` exécute une seule fois la garde de cible, la sécurité mutable autorisée, le SEO hébergé et les cinq specs Playwright de préproduction. Un sous-runner omis, dupliqué ou inattendu fait échouer le contrôle. Les exigences non encore démontrées reçoivent explicitement `not_run` avec action requise : elles restent visibles et conservent `promotionDecision=not_approved`.
