...
Une migration créée par la CLI renforce l'exactitude des prix, consolide les politiques `SELECT` des prestations et reprend les huit contenus actuels avec des identifiants et ordres déterministes. Aucun nouveau service externe, Route Handler, ORM, système de catégories, upload d'image ou parcours de réservation n'est introduit.
...
**Storage**: Table existante `public.prestations`; migration versionnée pour `prix numeric` borné à deux décimales, reprise des huit prestations et consolidation des politiques `SELECT`; aucune nouvelle table ni fichier Storage
...
**Scale/Scope**: un administrateur, environ 40 prestations, trois catégories fermées, trois types de prix, trois routes administratives principales, quatre Server Actions et un tag de cache public
...
| --- | --- | --- |
| Périmètre MVP | PASS | Le plan couvre uniquement le CRUD des prestations et leur lecture publique ; galerie, réservation, paiement, catégories administrables et opérations groupées restent exclus. |
...
│   ├── (public)/
│   │   └── services/
│   │       ├── page.tsx                     # Trois univers fixes + données publiques
...
**Structure Decision**: Conserver l'application unique et les groupes de routes actuels. La route publique garde `ServiceSection` et ses trois univers codés par le produit. Les nouvelles routes administratives sont placées sous le layout protégé existant. La liste reste un Server Component et délègue le pending de visibilité à `service-visibility-form.tsx`, frontière cliente imbriquée ; les formulaires, le dialogue et les `error.tsx` sont les autres frontières clientes nécessaires. La logique métier et les lectures restent sous `lib/`; aucune connexion Supabase, validation ou règle de prix n'entre dans un composant de présentation.

**Data Boundaries**: `getPublicServices()` utilise exclusivement un client anonyme sans cookies, sélectionne des colonnes explicites, filtre `actif = true`, retourne des DTO publics et porte `use cache`, `cacheLife("days")`, `cacheTag("prestations")`. `getAdminServices()` et `getAdminService()` utilisent le client SSR par requête après `requireAdminPage()`, sélectionnent uniquement les champs administrables et ne sont jamais cachés. Le rang des catégories est appliqué par les constantes produit, pas par l'ordre lexical SQL.

**Mutation Boundaries**: `createServiceAction`, `updateServiceAction`, `setServiceVisibilityAction` et `deleteServiceAction` suivent toutes la chaîne `requireAdminAction()` → validation Zod → mutation ciblée sous RLS → vérification de la ligne affectée → `updateTag("prestations")` → état structuré ou redirection. Une cible absente ou masquée par l'autorisation n'est jamais annoncée comme succès. La confirmation de suppression est une barrière UX ; l'action revérifie toujours UUID, session et droit.
...
- [data-model.md](./data-model.md) décrit `public.prestations`, les modèles applicatifs, l'état de visibilité, les états d'action et la reprise des huit lignes.
...
- [contracts/service-actions.md](./contracts/service-actions.md) fixe les quatre Server Actions, leurs entrées, sorties, autorisations, erreurs et invalidations.
...
| --- | --- | --- |
| Périmètre MVP | PASS | Quatre mutations unitaires et trois routes de gestion couvrent seulement les prestations ; toutes les extensions hors MVP sont absentes. |
