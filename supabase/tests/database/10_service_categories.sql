...
select extensions.is((select count(*) from pg_policies where schemaname = 'public' and tablename = 'categories_prestations'), 4::bigint, 'categories.rls.four_policies');
...
select extensions.is(
  ...
  'categories.grants.authenticated_crud'
);
...
select extensions.ok(tests.mutation_is_blocked('anon', $$update public.categories_prestations set nom = 'Anonyme' where code = 'onglerie_manucure'$$), 'categories.rls.anon_update_blocked');
select extensions.ok(tests.mutation_is_blocked('anon', $$delete from public.categories_prestations where code = 'onglerie_manucure'$$), 'categories.rls.anon_delete_blocked');
...
select extensions.ok(tests.mutation_is_blocked('authenticated', $$update public.categories_prestations set nom = 'Membre' where code = 'onglerie_manucure'$$), 'categories.rls.member_update_blocked');
select extensions.ok(tests.mutation_is_blocked('authenticated', $$delete from public.categories_prestations where code = 'onglerie_manucure'$$), 'categories.rls.member_delete_blocked');
...
select extensions.lives_ok($$update public.categories_prestations set nom = 'Massages relaxants', ordre_affichage = 5 where code = 'category_massages'$$, 'categories.rls.admin_update_allowed');
select extensions.results_eq($$select nom, ordre_affichage from public.categories_prestations where code = 'category_massages'$$, $$values ('Massages relaxants'::text, 5)$$, 'categories.rls.admin_update_persisted');
...
select extensions.throws_ok($$insert into public.categories_prestations (code, nom) values ('category_duplicate', '  MASSAGES RELAXANTS  ')$$, '23505', null, 'categories.schema.duplicate_name_rejected');
select extensions.throws_ok($$delete from public.categories_prestations where code = 'category_massages'$$, '23503', null, 'categories.fk.referenced_category_delete_rejected');
select extensions.lives_ok($$delete from public.prestations where categorie = 'category_massages'$$, 'categories.fk.referencing_service_delete_allowed');
select extensions.lives_ok($$delete from public.categories_prestations where code = 'category_massages'$$, 'categories.rls.admin_empty_category_delete_allowed');
select extensions.is((select count(*) from public.categories_prestations where code = 'category_massages'), 0::bigint, 'categories.rls.admin_delete_persisted');
reset role;
