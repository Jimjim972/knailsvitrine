...
const first = spawnSync(process.execPath, ["scripts/bootstrap-gallery.mjs", "--local"], { stdio: "inherit" }); if (first.status !== 0) process.exit(first.status ?? 1);
const second = spawnSync(process.execPath, ["scripts/bootstrap-gallery.mjs", "--local"], { encoding: "utf8" }); if (second.status !== 0 || second.stdout.split("\n").filter((line) => line.startsWith("unchanged ")).length !== GALLERY_BOOTSTRAP_ITEMS.length) throw new Error("Bootstrap is not idempotent");
